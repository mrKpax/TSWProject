...
public class OrderDetailsServlet extends HttpServlet{
    ...
    static ProductDAO ProductModel = new ProductDAO();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        if(action==null){
            ...
            if(order.getClient().getUsername().equalsIgnoreCase(client.getUsername()) || client.getEmail().equals("WorkInProgress@gmail.com")){
        ...
        if(action!=null && action.equalsIgnoreCase("viewInvoice")){
            ...
            ArrayList<ProductBean> Products = new ArrayList<ProductBean>(); // dati necessari alla creazione della fattura
            for(OrderProductBean prodotto : products){
                ProductBean Product = null;
				try {
					Product = ProductModel.doRetrieveByKey(prodotto.getId_prodotto());
				} catch (SQLException e) {
	                LOGGER.log( Level.SEVERE, e.toString(), e );
                    response.sendRedirect("generalError.jsp");
                    return;
